  ...
  if (!duplicatedUser) {
    ...
    if (registerResult) {
      ...
      res
        .status(500)
        .send(
          '회원가입 실패! 관리자에게 문의 바랍니다. <br> <a href="/register">회원가입으로 이동</a> <br><a href="/login">로그인으로 이동</a> ',
        );
    }
  } else {
    res
      .status(400)
      .send(
        '회원가입 실패! 동일한 ID를 가진 회원이 존재합니다 <br><a href="/register">회원가입으로 이동</a><br><a href="/login">로그인으로 이동</a>',
      );
  }
});
